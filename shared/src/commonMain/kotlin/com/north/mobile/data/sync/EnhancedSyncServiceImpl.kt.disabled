...
/**
 * Enhanced implementation of SyncService with comprehensive notification support,
 * improved error handling, and better background sync management
 */
class EnhancedSyncServiceImpl(
    ...
    private val syncTimeouts = mutableMapOf<String, Job>()
    
    companion object {
        private const val SYNC_TIMEOUT_MINUTES = 5L
        private const val INCREMENTAL_SYNC_THRESHOLD_MINUTES = 15L
        private const val BACKGROUND_SYNC_INTERVAL_MINUTES = 60L
    }
    
    override suspend fun syncAllAccounts(userId: String): SyncResult {
        val startTime = Clock.System.now()
        
        // Cancel any existing sync for this user
        cancelSync(userId)
        
        val syncJob = coroutineScope.launch {
            performSyncAllAccounts(userId, startTime)
        }
        
        activeSyncJobs[userId] = syncJob
        
        // Set up timeout notification
        val timeoutJob = coroutineScope.launch {
            delay(SYNC_TIMEOUT_MINUTES.minutes)
            if (syncJob.isActive) {
                notificationManager.notifySyncDelayed(userId, "all")
            }
        }
        syncTimeouts[userId] = timeoutJob
        ...
        return try {
            ...
            timeoutJob.cancel()
            syncTimeouts.remove(userId)
        }
    }
    
    private suspend fun performSyncAllAccounts(userId: String, startTime: Instant): SyncResult {
        return try {
            ...
            if (accountsResult.isFailure) {
                val error = SyncError.UnknownError("Failed to fetch user accounts", accountsResult.exceptionOrNull())
            ...
            if (activeAccounts.isEmpty()) {
                return SyncResult.Success(0, 0, 0, 0, Clock.System.now().minus(startTime).inWholeMilliseconds)
            }
            
            val syncResults = mutableListOf<SyncResult>()
            
            // Sync each account with parallel processing for better performance
            val accountSyncJobs = activeAccounts.map { account ->
                coroutineScope.async {
                    syncAccount(account.id)
                }
            }
            
            // Wait for all account syncs to complete
            accountSyncJobs.forEach { job ->
                val accountSyncResult = job.await()
            ...
            when (result) {
                is SyncResult.Success -> {
                    if (result.transactionsAdded > 0 || result.accountsUpdated > 0) {
                        notificationManager.notifySyncSuccess(userId, result)
                    }
                    if (result.conflictsResolved > 0) {
                        notificationManager.notifyConflictsResolved(userId, result.conflictsResolved)
                    }
                }
                is SyncResult.PartialSuccess -> {
                    notificationManager.notifySyncPartialSuccess(userId, result)
                    if (result.conflictsResolved > 0) {
                        notificationManager.notifyConflictsResolved(userId, result.conflictsResolved)
                    }
                    
                    // Check for authentication errors and send specific notifications
                    errors.filterIsInstance<SyncError.AuthenticationError>().forEach { authError ->
                        val account = accounts.find { it.id == authError.accountId }
                        if (account != null) {
                            notificationManager.notifyReauthRequired(userId, account.id, account.institutionName)
                        }
                    }
                }
                is SyncResult.Failure -> {
                    // This case is handled in the catch block
                }
            }
    ...
    override suspend fun syncAccount(accountId: String): SyncResult {
        ...
        return try {
            ...
            // Sync account balance with retry
            try {
                val balanceResult = retryManager.withRetry {
                    plaidService.getBalances(accessToken)
                }
                ...
                if (balanceResult.isSuccess) {
                    ...
                    val exception = balanceResult.exceptionOrNull()
                    val error = mapPlaidErrorToSyncError(exception, accountId)
                    errors.add(error)
                }
            } catch (e: Exception) {
            ...
            // Record sync completion for status tracking
            syncStatusManager.recordSyncCompletion(accountId, result)
    ...
    override suspend fun syncTransactions(
        ...
        return try {
            ...
            // Fetch transactions from Plaid with retry
            ...
            if (transactionResult.isFailure) {
                val exception = transactionResult.exceptionOrNull()
                val error = mapPlaidErrorToSyncError(exception, accountId)
    ...
    override suspend fun incrementalSync(userId: String): SyncResult {
        ...
        return try {
            ...
            val syncThreshold = now.minus(INCREMENTAL_SYNC_THRESHOLD_MINUTES.minutes)
            ...
            // Only send notifications for incremental sync if there are significant changes
            when (result) {
                is SyncResult.Success -> {
                    if (result.transactionsAdded > 5) { // Only notify for significant transaction additions
                        notificationManager.notifyNewTransactions(userId, "multiple", result.transactionsAdded)
                    }
                }
                is SyncResult.PartialSuccess -> {
                    if (result.transactionsAdded > 5) {
                        notificationManager.notifyNewTransactions(userId, "multiple", result.transactionsAdded)
                    }
                }
                is SyncResult.Failure -> {
            ...
            syncStatusManager.updateUserSyncStatus(userId, SyncStatus.ERROR)
            val error = SyncError.UnknownError("Incremental sync failed", e)
            notificationManager.notifySyncFailure(userId, error)
            SyncResult.Failure(
                error,
                Clock.System.now().minus(startTime).inWholeMilliseconds
    ...
    override suspend fun cancelSync(userId: String) {
        ...
        activeSyncJobs.remove(userId)
        syncTimeouts[userId]?.cancel()
        syncTimeouts.remove(userId)
        syncStatusManager.updateUserSyncStatus(userId, SyncStatus.CANCELLED)
        notificationManager.cancelNotifications(userId)
    }
        ...
        val job = coroutineScope.launch {
            while (isActive) {
                try {
                    ...
                    val result = incrementalSync(userId)
                    
                    // Schedule next sync based on result
                    val nextSyncTime = Clock.System.now().plus(intervalMinutes.minutes)
                    val accounts = accountRepository.findByUserId(userId).getOrElse { emptyList() }
                    accounts.forEach { account ->
                        syncStatusManager.scheduleNextSync(account.id, nextSyncTime)
                    }
    ...
    private fun mapPlaidErrorToSyncError(exception: Throwable?, accountId: String): SyncError {
        return when (exception) {
            ...
            else -> SyncError.UnknownError("Failed to sync", exception)
        }
    }
}